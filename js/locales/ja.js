const translationsJa = {
    nav: { hero: "Home", vision: "Vision", concept: "Konzept", artists: "Artists", schedule: "Schedule", access: "Access" },
    hero: {
        date: "10:00–20:00 / 23.05.2026 | 10:00-17:00 / 24.05.2026",
        entry: "入場無料。事前予約が必要です。",
        venue: "日独センター ベルリン, Saargemünder Str. 2, 14195 Berlin, Germany",
        btn: "Register Now",
        url: "https://rausgegangen.de/en/events/shu-ha-ri-2026-2/",
        img: "images/main-b.jpeg" // 背景画像
    },
    vision: {
        main: "文化の過程をひらき、解釈が生まれ続ける循環をつくる。",
        sub: "守破離は、文化を知識や完成されたものとして留めるのではなく、<br>受け取り、揺さぶり、再解釈し、<br>自らの創造へとつなげていくためのプロセスです。<br>私たちはこのイベントを通して、<br>文化が「見るもの」や「学ぶもの」で終わるのではなく、<br>次の表現や対話へと手渡されていく、<br>ひとつの〈場〉をつくりたいと考えています。"
    },
    concept: {
        title: "あなたの中で完成する日本文化。「守破離」を辿ろう。",
        body: "日本文化は、<br>ショーケースの中で完成するものではありません。<br>人々の暮らしの中で、時間とともに姿を変えながら、<br>生き続けてきました。<br>私たちは、特定の日本文化が完成されたイメージとして語られ、<br>そこに至るまでの細部に宿る思考や、積み重ねられてきた過程、<br>これらが十分に共有されていない現状に課題を感じてきました。<br>内側にいたときには見えにくかった日本のかたち。<br>距離を置いたことで浮かび上がった、細やかさや余白。<br>本イベントは、型を学び、殻を破り、自らの表現へと昇華させる<br>「守破離」というプロセスを、追体験する場です。<br>作品の完成形ではなく、その過程に触れることで、<br>来場者一人ひとりの中に新しい解釈が立ち上がることを目指します。"
    },
    artists: {
        list: [
            { id: "01", name: "Performance", desc: "Live Performance & Dance", img: "./images/small_performance.png", color: "shu" },
            { id: "02", name: "Exhibition", desc: "Creative Exhibition with 40+ Artists", img: "./images/small_exhivition.png", color: "ha" },
            { id: "03", name: "Workshop", desc: "Interactive Cultural Workshops", img: "./images/small_workshop.png", color: "ri" },
            { id: "04", name: "Food & Drink", desc: "Authentic Japanese Food Experience", img: "./images/small_japanesefood&drink.png", color: "shu" }
        ]
    },
    social: {
        instagram: "https://www.instagram.com/shuhari_berlin/",
        facebook: "https://www.facebook.com/share/1bGK4AHXQm/?mibextid=wwXIfr"
    },
    schedule: { events: [{ time: "upcoming", name: "upcoming" }] },
    access: { venue: "日独センター　ベルリン", address: "Saargemünder Str. 2, 14195 Berlin", note: "Admission Free" },
    support: [
        { name: "Tobitate! Study Abroad Japan", img: "images/logo_tobitate.png"},
        { name: "Japanisch-Deutsches Zentrum Berlin", img: "images/logo_jdzb.png"}
    ]
};